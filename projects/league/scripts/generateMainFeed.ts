...
import { PROFILES_DIR, FEED_MAIN_FILE } from "../dirs";
import stopWords from "../stop-words/stopWords";
...
(async function (): Promise<void> {
  ...
  for (const profile of profiles) {
    ...
    const videos: YouTubeVideo[] = origVideos
      .filter((video: any) => {
        const checkString = (
        ...
        for (const stopWord of stopWords) {
          if (checkString.indexOf(stopWord) !== -1) {
            return false;
          }
        }
